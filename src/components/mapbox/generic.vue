...
export default {
  ...
  mounted() {
    ...
    function updateArea(e) {
      ...
      const payload = {
        operationName: "fetchNearbyRestaurants",
        variables: { bound: 1000, lat: lat, long: lon },
        query:
      ...
      axios.post("https://api.geoql.in/v1/graphql", payload).then((res) => {
        if (
          res.data &&
          res.data.data &&
          res.data.data.get_nearby_restaurants &&
          res.data.data.get_nearby_restaurants.length
        ) {
          res.data.data.get_nearby_restaurants.forEach((data) => {
            this.marker = new Mapbox.Marker()
              ...
              .addTo(map);
            scope.allMarkers.push(this.marker);
          });
        }
      });
    }
  ...
  methods: {
    onChange: function(event) {
      ...
      if (event.target.value === "Satellite")
        mapStyle = "mapbox://styles/kunvar/ckodw2d563dqm17oeio2l5q5s";
      if (event.target.value === "Dark")
        mapStyle = "mapbox://styles/kunvar/ckodvr00f3fry17noagemspc7";
